...
# -*- coding: utf-8 -*-
"""
Created on Sun May 24 13:49:46 2020
...
#  série de óbitos, data do primeiro caso e população
#
    Y = np.zeros(1000)
    YD = np.zeros(1000)
...
for reg in reglist: 
    ...
    R_raw,D_raw,N_k,First_Day,Popul = read_csv_data(reg)
    csvfile.close()
...
for i in range (0,len(reglist)):      
    plt.plot(T,Y[:,arg[len(reglist)-1-i]],label = reglist[arg[len(reglist)-1-i]]+", r0 = "+'{0:.2f}'.format(R0list[arg[len(reglist)-1-i]]))
...
plt.ylabel("Novos casos")
plt.yscale("log")
plt.title("Menor crescimento médio nas últimas semanas \n "+date[6:8]+"/"+date[4:6]+"/"+date[0:4])
...
for i in range (0,5):
    reg = reglist[arg[4-i]]
    plt.plot(T,Y[:,arg[4-i]],label = reg+", r0 = "+'{0:.2f}'.format(R0list[arg[4-i]]))
...
for i in range (0,5):
    ...
    plt.plot(T,Y[:,arg[len(reglist)-1-i]],label = reg+", r0 = "+'{0:.2f}'.format(R0list[arg[len(reglist)-1-i]]))
...
for reg in reglist:
    js_file.write("      data.addColumn('number', '"+reg+", r0 = "+'{0:.2f}'.format(R0list[reglist.index(reg)])+"'); \n")
js_file.write(' \n')
...
js_file.write("    var selectedLineWidth = 5;\n")
